...
@section('meta_keywords','Travel, Tour')
@section('meta_description','Lorem ipsum dolor sit amet consectetur adipisicing elit. Consectetur, maiores.')
@section('content')
<!-- =======================
Main Banner START -->
<style>
	...
	<div class="container position-relative">
		...
		<div class="row mt-n7">
			<div class="col-11 mx-auto">
				<!-- Booking from START -->
				 <form class="bg-mode shadow rounded-3 p-4" action="{{ route('tour.search') }}" method="GET">
...
<section class="pt-0 pt-md-5">
	<div class="container">
		...
		<div class="row g-4">
			<!-- Package item -->
			 @if (count($featured_tour)>0)
			 @foreach ( $featured_tour as $featured)
				<div class="col-sm-6 col-xl-3">
					...
					<div class="position-relative">
						...
						<img src="{{ asset($featured->featured_image) }}" class="card-img-top" alt="Card image">
						@else
						<p>N/A</p>
						@endif
						<!-- Overlay -->
						<div class="card-img-overlay d-flex flex-column p-4 z-index-1">
							<!-- Card overlay top -->
							<div> <span class="badge text-bg-dark">{{ $featured->location }}</span> </div>
							<!-- Card overlay bottom -->
							<div class="w-100 mt-auto">
								<!-- Card category -->
								<span class="badge text-bg-white fs-6">{{ $featured->duration }}</span>
							</div>
						</div>
					</div>
					<!-- Image -->
					...
					<div class="card-body px-3">
						<!-- Title -->
						<h5 class="card-title mb-0"><a href="{{ route('tour.details',$featured->slug) }}" class="">{{ $featured->title }}</a></h5>
						<span class="small">{{ $featured->category->name }}</span>
					</div>
					<!-- Card body END -->
						...
						<div class="d-sm-flex justify-content-sm-between align-items-center flex-wrap">
							...
							<div class="gap-2">
								...
								@if($featured->discount_price && $featured->discount_price < $featured->price)
									<h5 class="fw-normal text-success mb-0">{{ $currency }}{{ number_format($featured->discount_price, 2) }}</h5>
									<span class="text-decoration-line-through text-muted">{{ $currency }}{{ number_format($featured->price, 2) }}</span>
								@else
									<h5 class="fw-normal text-success mb-0">{{ $currency }}{{ number_format($featured->price, 2) }}</h5>
								@endif
							</div>
							<!-- Button -->
							<div class="mt-2 mt-sm-0">
								<a href="{{ route('tour.details',$featured->slug) }}" class="btn btn-sm btn-primary mb-0">View Details</a>    
							</div>
						</div>
					</div>
		...
		<div class="bg-light rounded-3 position-relative p-3 p-sm-4">
			...
			<div class="row">
				<div class="col-md-7 mx-auto text-center py-5">
                    ...
                    @endif
					<form class="bg-body shadow rounded-2 p-2" action="{{ route('newsletter.store') }}" method="POST">
						@csrf
						<div class="input-group">
							<input class="form-control border-0 me-1" type="email" name="email" placeholder="Enter your email">
							<button type="submit" class="btn btn-dark rounded-2 mb-0">Subscribe!</button>
						</div>
					</form>
				</div>
			</div>
			...
			<div class="col-sm-6 col-lg-4 col-xl-3">
				<div class="row g-2 g-md-3 align-items-center position-relative">
					...
					<div class="col-md-6">
						...
						<img src="{{ asset($category->image) }}" class="rounded-3" alt="{{ $category->name }}">
						@endif
					</div>
					<!-- Content -->
					<div class="col-md-6">
						<div class="p-2 p-md-0">
							<h5 class="mb-1"><a href="{{ route('tour.category',$category->slug) }}" class="stretched-link">{{ $category->name }}</a></h5>
							<span>{{ $category->tours_count }} Places</span>
						</div>
					</div>
				</div>
			</div>
	...
	<div class="container">
		...
		<div class="row g-4 g-lg-5 align-items-center">
			...
			<div class="col-lg-4">
				...
				<!-- Button -->
				<a href="{{ route('tours') }}" class="btn btn-lg btn-primary-soft mb-0 mt-4">Explore places <i class="bi fa-fw bi-arrow-right ms-2"></i></a>
			</div>
				...
				<div class="row">
					<div class="col-md-6">
						...
						<div class="card card-element-hover card-overlay-hover overflow-hidden">
							<!-- Image -->
							<img src="http://localhost:8000/images/jYpRG4MNm2IiRpuhi31TYSW3VM0PuvVAATAph67u.jpg" class="rounded-3" alt="">
							<!-- Full screen button -->
							<a class="hover-element position-absolute w-100 h-100 stretched-link" data-glightbox data-gallery="gallery" href="http://localhost:8000/images/jYpRG4MNm2IiRpuhi31TYSW3VM0PuvVAATAph67u.jpg">
								...
								<!-- Testimonial item -->
								 ...
								 <div class="card bg-transparent">
									<div class="row align-items-center">
										...
										<div class="col-4">
											@if (!empty($blog->image))
											<img src="{{ asset($blog->image) }}" class="card-img home-blog-image" alt="{{ $blog->title }}">
											@endif
										</div>
										<!-- Card body -->
										<div class="col-8">
											<div class="card-body p-0">
												...
												<h6 class="card-title mb-0 mt-2">{{ $blog->author }}</h6>
											</div>
										</div>
									</div>
								</div>
					...
					<div class="col-md-6">
						...
						<div class="card card-element-hover card-overlay-hover overflow-hidden mb-4">
							<!-- Image -->
							<img src="http://localhost:8000/images/a4rxf9vpPl0NS4qyfz058bxkb8Uh7mlLk14mS9ez.jpg" class="rounded-3" alt="">
							<!-- Full screen button -->
							<a class="hover-element position-absolute w-100 h-100 stretched-link" data-glightbox data-gallery="gallery" href="https://youtu.be/mkdGSlmLO6U?si=BAzGFb8ScbA8PJfC">
						...
						<div class="card card-element-hover card-overlay-hover overflow-hidden">
							<!-- Image -->
							<img src="http://localhost:8000/images/ilFzzI8e0f57H5aQBW0fzRS69dAsjO2ktkrXADaZ.jpg" class="rounded-3" alt="">
							<!-- Full screen button -->
							<a class="hover-element position-absolute w-100 h-100 stretched-link" data-glightbox data-gallery="gallery" href="http://localhost:8000/images/ilFzzI8e0f57H5aQBW0fzRS69dAsjO2ktkrXADaZ.jpg">
								<i class="bi bi-fullscreen fs-6 text-white position-absolute top-50 start-50 translate-middle bg-dark rounded-1 p-2 lh-1"></i>
							</a>
						</div>
					</div>
